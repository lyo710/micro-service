...
public class MethodExecutorImpl implements MethodExecutor {
    ...
    public <T> T execute(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (arg.getClass() != type) {
                args[i] = ProtostuffReflect.transform(type, arg);
            }
        }
    ...
    private Class<? extends MethodExecutor> createDelegateClass()
        ...
        StringBuilder execute =
                new StringBuilder("{").append("void".equals(method.getReturnType().getTypeName()) ? "" : "return ")
                        .append("this.target.")
        ...
        execute.append(");").append("void".equals(method.getReturnType().getTypeName()) ? "return null;}" : "}");
